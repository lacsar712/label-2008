...
if (empty($ids) || !is_array($ids)) {
    json_response(400, '请选择要操作的Banner');
}
...
$before_stmt = $conn->prepare("SELECT * FROM banners WHERE id IN ($placeholders_select)");
...
$stmt = $conn->prepare("UPDATE banners SET status = ? WHERE id IN ($placeholders)");
...
if ($stmt->execute()) {
    ...
    $after_stmt = $conn->prepare("SELECT * FROM banners WHERE id IN ($placeholders_select)");
    ...
    write_operation_log('batch_update', 'banner', $ids, [
        'banners' => $before_data
    ], [
        'banners' => $after_data,
        'new_status' => $status
